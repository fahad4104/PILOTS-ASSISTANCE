...
export async function POST(req: Request) {
  ...
  try {
    ...
    // 1) Upload to OpenAI Files (from blob)
    ...
    if (!openaiFileId) {
      return NextResponse.json({ error: "OpenAI files.create returned no id" }, { status: 500 });
    }

    // 2) Attach to Vector Store (index)
    ...
    return NextResponse.json({
      ...
      vector_store_file_id: vsFile?.id ?? null,
    });
  } catch (e: any) {
    return NextResponse.json(
